...
class tx_dataquery_sqlbuilder_Workspace_Test extends tx_dataquery_sqlbuilder_Test {
	...
	public function setUp() {
		...
		self::$minimalConditionForTTContent = 'tt_content.deleted=0';
		self::$baseConditionForTTContent = '(###MINIMAL_CONDITION###)';
		self::$groupsConditionForTTContent = '';
			// Reset language condition which might have been altered by language unit test
		self::$baseLanguageConditionForTTContent = '(tt_content.sys_language_uid IN (0,-1))';
			// Add workspace condition, assuming some arbitrary workspace (= 42)
		self::$baseWorkspaceConditionForTTContent = '((tt_content.t3ver_state <= 0 AND tt_content.t3ver_oid = 0) OR (tt_content.t3ver_state = 0 AND tt_content.t3ver_wsid = 42) OR (tt_content.t3ver_state = 1 AND tt_content.t3ver_wsid = 42) OR (tt_content.t3ver_state = 3 AND tt_content.t3ver_wsid = 42)) ';
